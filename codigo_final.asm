...
# la   : Load Address (Carrega o endere√ßo de mem√≥ria de uma vari√°vel/r√≥tulo)
# add  : Soma dois registradores
# addi : Soma um registrador com uma constante (Immediate)
# sub  : Subtra√ß√£o
# sll  : Shift Left Logical (Desloca bits √† esquerda = Multiplica por pot√™ncias de 2)
...
# j    : Jump (Salto incondicional para um r√≥tulo)
# jal  : Jump and Link (Salta para sub-rotina e salva o endere√ßo de retorno)
# jr   : Jump Register (Retorna de uma sub-rotina usando o registrador $ra)
# div  : Divis√£o (Quociente vai para 'lo', resto para 'hi')
# mflo : Move from LO (Move o quociente da divis√É¬£o para um registrador)
# mfhi : Move from HI (Move o resto da divis√É¬£o para um registrador)
# syscall : Chamada ao sistema operacional (I/O, arquivos, encerrar)
#
# REGISTRADORES COMUNS:
# $v0, $v1 : C√≥digos de opera√ß√£o para syscalls e retornos de fun√ß√µes
# $a0-$a3  : Argumentos passados para fun√ß√µes ou syscalls
# $t0-$t9  : Tempor√°rios (podem ser sobrescritos livremente)
# $s0-$s7  : Salvos (devem ser preservados ou restaurados)
# $sp      : Stack Pointer (Ponteiro da pilha)
# $ra      : Return Address (Guarda endere√ßo de retorno do jal)

# =============================================================================
# JOGO DA COBRINHA (SNAKE) EM ASSEMBLY MIPS - VERS√O FINAL (TEP)
# =============================================================================

.data

# Arrays paralelos para o corpo da cobra (Aumentados para caber na tela 64x64)
...
# Variaveis para a Integracao Web (score.bin) - CAMINHO ABSOLUTO LOCAL
arquivo_score: .asciiz "C:/Users/arthu/OneDrive/¡rea de Trabalho/snake/score.bin"
buffer_score:  .word 0
...
velocidade:   .word 400        # ComeÁa em 400ms
...
main:
    ...
    # Atualiza o display para mostrar "0" antes do jogo comeÁar
    jal  atualizar_display

# --- TELA DE ESPERA (Aguarda ESPA«O) ---
tela_espera:
    li   $t1, 0xFFFF0000
    lw   $t0, 0($t1)
    andi $t0, $t0, 1
...
# =============================================================================
# FUNCAO: escrever_pixel (SLL Otimizado 1 ciclo de clock)
# =============================================================================
escrever_pixel:
...
inicializar_cobra:
    ...
    li   $t2, 32            # Cobra comeÁa no meio da nova tela (X=32, Y=32)
    sw   $t2, 0($t0)
    sw   $t2, 0($t1)
...
verificar_maca:
    ...
    # Atualiza o display quando pontua
    jal  atualizar_display
...
# =============================================================================
# FUNCAO: atualizar_display (CorreÁ„o do EspaÁo)
# =============================================================================
atualizar_display:
    lw   $t0, cobra_tamanho
    addi $t0, $t0, -3          # O placar real È o tamanho - 3
...
# =============================================================================
# FUNCAO: game_over (IntegraÁ„o Web)
# =============================================================================
game_over:
    lw   $t0, cobra_tamanho
    li   $t1, 0

    # --- INTEGRA«√O WEB (GeraÁ„o do score.bin) ---
